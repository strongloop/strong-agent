...
Agent.prototype.use = function(callback) {
  if (!this.config.license.allows('metrics')) {
    this.notice([
      (this.config.key
...
Agent.prototype.metrics = (function(agent) {
  return {
    ...
    stopCpuProfiling: function() {
      ...
      agent.info('stopping cpu profiler');
      return profile;
    },
    startTrackingObjects: function() {
      if (!agent.config.license.allows('metrics')) {
