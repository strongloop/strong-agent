...
Agent.prototype.start = function() {
  ...
  proxy.init(this);
  sender.init(this);
  counts.init(this, config.baseInterval);
  cpuProf.init(this);
  info.init(this, config.baseInterval);
  metrics.init(this, config.baseInterval);
  tiers.init(this, config.baseInterval);
  loopbackTiers.init(this, config.baseInterval);
  loop.init(this, config.baseInterval);
  this.dyninst.metrics = dyninstMetrics.init(this, dyninst);
  topFunctions.init(this, config.baseInterval);
...
Agent.prototype.prepareProfilers = function() {
  var self = this;
  memProf.init(this, this.config.baseInterval);
...
Agent.prototype.use = function(callback) {
  ...
  this.internal.on('i::stats', function(stat, value, type) {
    switch (type) {
      case 'timer':
  ...
  this.internal.on('i::counts', function(counts) {
    var labels = {
  ...
  this.internal.on('i::send', function(name, value) {
    ...
    if (name === 'update' && typeof(value.tiers) === 'object') {
      Object.keys(value.tiers).forEach(function(key) {
        ...
        var name = (last < 0 ? key : key.slice(0, last));
...
Metrics.prototype.startCpuProfiling = function(timeout) {
  ...
  addon.startCpuProfiling(timeout);
};
