...
Agent.prototype.preparePoll = function(config) {
  setInterval(poll, config.baseInterval).unref();
  var agent = this;

  function poll() {
    var data;

    agent.emit('poll::start');

    if (data = counts.poll()) {
      agent.internal.emit('i::counts', data);
    }
    ...
    if (data = memProf.poll()) {
      agent.internal.emit('instances', {type: 'Instances', state: data});
    }

    if (data = tiers.poll()) {
      agent.internal.emit('tiers', {tiers: data});
    }

    if (data = loopbackTiers.poll()) {
      agent.internal.emit('loopback_tiers', {loopback_tiers: data});
    }
    ...
    if (data = metrics.poll()) {
      for (var key in data) {
        agent.internal.emit('metric', data[key]);
      }
    }

    agent.emit('poll::stop');
  }
};
