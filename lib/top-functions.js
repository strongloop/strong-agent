...
function add(collectionName, url, ms, tiers, graph) {
  var now = Date.now();
  // The unused field is for compatibility with the collector wire format.
  // It can safely be removed once we kill off the hosted version.
  var data = [now, url, ms, 0, tiers, graph];
  var update = false;
