...
function before(object, method, hook) {
  around(object, method, hook);
}

function after(object, method, hook) {
  around(object, method, null, hook);
}
...
function around(object, method, before, after) {
  ...
  if (isproxy(target)) {
    ...
    var des = first(all, function(_, des) {
      return des &&
             des.value &&
             des.value[kContextPropertyName] &&
...
function callback(args, index, before, after) {
  ...
  var proxy = wrap(target, function(recv, args) {
    ...
    if (after) before(recv, args, extra, graph, currentNode);
    // TODO(bnoordhuis) Get rid of these globals.
