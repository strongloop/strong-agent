...
exports.init = function(agent_, interval) {
  ...
  agent.internal.on('info', function(info) { infoBuffer = info; });

  agent.internal.on('metric', function(metric) { metricsBuffer.push(metric); });
  ...
  agent.internal.on('loopback_tiers',
                    function(stats) { tiersBuffer.push(stats); });

  agent.internal.on('callCounts', function(counts) {
    agent.internal.send('update', counts);
  });

  agent.internal.on('loop', function(loop) { loopBuffer.push(loop); });
...
function sendMetrics() {
  ...
  metricsBuffer.map(sanitizers.metrics).forEach(function(metric) {
    if (metric) agent.internal.send('update', metric);
  });
...
function sendTiers() {
  ...
  tiersBuffer.map(sanitizers.metrics).forEach(function(stats) {
    if (stats) agent.internal.send('update', stats);
  });
