...
tap.test('Memcached probe', maybeSkip, function(t) {
  ...
  client.set('key', '42', dataLifeInSeconds, function(err) {
    ...
    t.ok(ixAvg > -1 && metrics.length > ixAvg, 'Memcached avg metric exists.');
    t.ok(ixMin > -1 && metrics.length > ixMin, 'Memcached min metric exists.');
    t.ok(ixMax > -1 && metrics.length > ixMax, 'Memcached max metric exists.');
