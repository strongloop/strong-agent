...
function onrequest(req, res) {
 ...
 function onerror(err) {
   // Work around https://github.com/joyent/node/issues/5439, a streams2 bug
   // where core emits a 'stream.push() after EOF' error when two TCP packets
   // arrive on the same tick and where response.end() is called after
   // processing the first one but before the second one. As this is
   // out of our control, all we can do is swallow the error.
